    ...
    *size_out = usedsize;
...
#ifdef CRCDEBUG
    {
        ...
        for (i = 0; i < usedsize; ++i)
        {
            ...
            result = modes_checksum(msg, bits);
...
int main(int argc, char **argv)
{
    ...
    init_lookup_tables();
    shorttable = prepare_error_table(MODES_SHORT_MSG_BITS, atoi(argv[1]), atoi(argv[2]), &shortlen);
    longtable = prepare_error_table(MODES_LONG_MSG_BITS, atoi(argv[1]), atoi(argv[2]), &longlen);
