 ...
 */
inline int modes_message_len_by_type(int type)
{
 ...
 */
static int decode_id13_field(int ID13Field)
{
    int hexGillham = 0;
 ...
 */
static int decode_ac13_field(int AC13Field, altitude_unit_t *unit)
{
    ...
    if (!m_bit)
    {
        *unit = UNIT_FEET;
        ...
        else
        {
            ...
            int n = mode_a_to_mode_c(decode_id13_field(AC13Field));
            if (n < -12)
            {
...
static int decode_ac12_field(int AC12Field, altitude_unit_t *unit)
{
...
static float decode_movement_field_v2(unsigned movement)
{
    ...
    if (movement >= 125)
        ...
        return 180; // gs > 175kt, pick a value..
    else if (movement >= 109)
        return 100 + (movement - 109 + 0.5) * 5; // 100 < gs <= 175 in 5kt steps
    else if (movement >= 94)
        return 70 + (movement - 94 + 0.5) * 2; // 70 < gs <= 100 in 2kt steps
    else if (movement >= 39)
        return 15 + (movement - 39 + 0.5) * 1; // 15 < gs <= 70 in 1kt steps
    else if (movement >= 13)
        return 2 + (movement - 13 + 0.5) * 0.50; // 2 < gs <= 15 in 0.5kt steps
    else if (movement >= 9)
        return 1 + (movement - 9 + 0.5) * 0.25; // 1 < gs <= 2 in 0.25kt steps
    else if (movement >= 3)
        return 0.125 + (movement - 3 + 0.5) * 0.875 / 6; // 0.125 < gs <= 1 in 0.875/6 kt step
    else if (movement >= 2)
        return 0.125 / 2; // 0 < gs <= 0.125
    // 1: stopped, gs = 0
    // 0: no data
    else
        return 0;
...
static float decode_movement_field_v0(unsigned movement)
{
    // Note : movement codes 0,125,126,127 are all invalid, but they are
    //        trapped for before this function is called.
    ...
    if (movement >= 125)
        ...
        return 0.125 + (movement - 2 + 0.5) * 0.125; // 0.125 < gs <= 1 in 0.125kt step
...
int score_modes_message(unsigned char *msg, int validbits)
{
    ...
    msgbits = modes_message_len_by_type(msgtype);
    ...
    crc = modes_checksum(msg, msgbits);
    ...
    case 11: // All-call reply
        ...
        ei = modes_checksum_diagnose(crc, msgbits);
        if (!ei)
 ...
 */
static void decode_extended_squitter(modes_message_t *mm);
...
int decode_modes_message(modes_message_t *mm, unsigned char *msg)
{
    ...
    case 11: // All-call reply
        ...
        {
            int addr;
            struct errorinfo *ei = modes_checksum_diagnose(mm->crc & 0xffff80, mm->msgbits);
            if (!ei)
            {
    ...
    { // Extended squitter/non-transponder
        ...
        if (mm->crc != 0)
        {
            ei = modes_checksum_diagnose(mm->crc, mm->msgbits);
            if (!ei)
            {
                return -2; // couldn't fix it
            }
    ...
    if (mm->msgtype == 0 || mm->msgtype == 4 || mm->msgtype == 16 || mm->msgtype == 20)
    {
        ...
        { // Only attempt to decode if a valid (non zero) altitude is present
            mm->altitude_baro = decode_ac13_field(mm->AC, &mm->altitude_baro_unit);
    ...
    if (mm->msgtype == 5 || mm->msgtype == 21)
    {
        ...
        if (mm->ID)
        {
            mm->squawk = decode_id13_field(mm->ID);
            mm->squawk_valid = 1;
        }
    }
...
static void decode_es_ident_category(modes_message_t *mm)
{
...
static void set_imf(modes_message_t *mm)
{
...
static void decode_es_airborne_velocity(modes_message_t *mm, int check_imf)
{
...
static void decode_es_surface_position(modes_message_t *mm, int check_imf)
{
...
static void decode_es_airborne_position(modes_message_t *mm, int check_imf)
{
    ...
    else
    {
        ...
        if (AC12Field == 0 && mm->cpr_lon == 0 && (mm->cpr_lat & 0x0fff) == 0 && mm->metype == 15)
        {
            ...
            lib_state.stats_current.cpr_filtered++;
        }
        else
        {
    ...
    if (AC12Field && mm->airground != AG_GROUND)
    { // Only attempt to decode if a valid (non zero) altitude is present and not on ground
        altitude_unit_t unit;
        int alt = decode_ac12_field(AC12Field, &unit);
...
static void decode_es_test_message(modes_message_t *mm)
{
    unsigned char *me = mm->ME;
    ...
    { // (see 1090-WP-15-20)
        ...
        {
            mm->squawk_valid = 1;
            mm->squawk = decode_id13_field(ID13Field);
        }
    }
}

static void decode_es_aircraft_status(modes_message_t *mm, int check_imf)
{
...
static void decode_es_target_status(modes_message_t *mm, int check_imf)
{
    unsigned char *me = mm->ME;
...
static void decode_es_operational_status(modes_message_t *mm, int check_imf)
{
    unsigned char *me = mm->ME;
...
static void decode_extended_squitter(modes_message_t *mm)
{
    ...
    case 3:
    case 4:
        decode_es_ident_category(mm);
        break;

    case 19:
        decode_es_airborne_velocity(mm, check_imf);
        break;
    ...
    case 16:
    case 17:
    case 18: // Airborne position, baro
    case 20:
    case 21:
    case 22: // Airborne position, geometric altitude (HAE or MSL)
        decode_es_airborne_position(mm, check_imf);
        break;

    case 23:
        decode_es_test_message(mm);
        break;
    ...
    case 28:
        decode_es_aircraft_status(mm, check_imf);
        break;

    case 29:
        decode_es_target_status(mm, check_imf);
        break;
    ...
    case 31:
        decode_es_operational_status(mm, check_imf);
        break;
...
void use_modes_message(modes_message_t *mm)
{
    ...
    ++lib_state.stats_current.messages_total;
